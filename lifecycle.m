...
function ensure_cetools()
    % 直接使用 CEtools；如果 MATLAB path 中没有，就尝试加入本机常见路径。
    if exist('fundef', 'file') == 2 && exist('funbas', 'file') == 2 && ...
            exist('funeval', 'file') == 2 && exist('newton', 'file') == 2 && ...
            exist('goldenx', 'file') == 2
        return
    end

    candidates = { ...
        'D:\code\Collocation\CEtools', ...
        'D:\CodeLibrary\Collocation\CEtools', ...
        'D:\CodeLibrary\compecon2011\CEtools', ...
        'D:\Collocation\CEtools', ...
        'D:\compecon2011_64_20110718\CEtools', ...
        'D:\Fortran_libs\compecon2011\CEtools'};
    ...
    needed = {'fundef', 'funbas', 'funeval', 'newton', 'goldenx'};
    missing = {};
    ...
    if ~isempty(missing)
        error('Missing CEtools functions on MATLAB path: %s', strjoin(missing, ', '));
    end
end
...
function par = initialize_model()
    ...
    par.pen(par.JR:par.JJ) = 0.5*par.w*sum(par.eff)/(par.JR-1);
    ...
    par.a_bor = borrowing_limits(par);

    % 预先计算劳动收入和现金在手，避免 Newton/黄金分割内部重复计算。
    par.labor_income = zeros(par.JJ, par.NP, par.NS);
    for j = 1:par.JJ
        for ip = 1:par.NP
            for is = 1:par.NS
                if j < par.JR
                    par.labor_income(j, ip, is) = ...
            ...
            for ip = 1:par.NP
                for is = 1:par.NS
                    par.cash_on_hand(j, ia, ip, is) = ...
                        (1+par.r)*(par.a(ia) + par.a_bor(j, ip)) + ...
                        par.labor_income(j, ip, is) + par.pen(j);
                end
            end
        end
    end
...
function sol = solve_collocation(par, col)
    ...
    for ip = 1:par.NP
        ...
        for is = 1:par.NS
            ...
            Vmat(:, is) = utility(Cmat(:, is), par); % 先计算最后一期的价值函数
        end
    ...
    for j = par.JJ-1:-1:1
        ...
        for ip = 1:par.NP
            ...
            [rhsV, c_policy, a_policy] = policy_rhs_age_ip(j, ip, sol(j+1), par, col);
...
function z0 = initial_guess(sol_next, ip, par, col)
    coefV0 = reshape(sol_next.coefV(:, ip, :), col.nx, par.NS);
    coefEV0 = reshape(sol_next.coefEV(:, ip, :), col.nx, par.NS);
    z0 = [coefV0(:); coefEV0(:)];
end
...
function [rhsV, c_policy, a_policy] = policy_rhs_age_ip(j, ip, sol_next, par, col)
    ...
    for is = 1:par.NS
        ...
        ap_high = resources - par.a_bor(j+1, ip) - par.c_min;
        ...
        a_policy(:, is) = ap;
        c_policy(:, is) = max(resources - (ap + par.a_bor(j+1, ip)), par.c_min);
        rhsV(:, is) = val;
    end
    rhsV = rhsV(:);
end

function val = bellman_rhs(aplus, resources, coefEV_next, j, ip, is, par, col) %#ok<INUSD>
    % Bellman 右端使用 Mongey 的普通 expected value：
    % u(c) + beta*psi(j+1)*EV_{j+1}(aplus,ip,is)。
    cons = max(resources - (aplus + par.a_bor(j+1, ip)), par.c_min);
    cont = funeval(coefEV_next, col.fspace_a, aplus);
    cont = reshape(cont, size(aplus));
    val = utility(cons, par) + par.beta*par.psi(j+1)*cont;
end
...
function a_bor = borrowing_limits(par)
    % 复刻 Fortran 的 endogenous borrowing constraint 逻辑。
    a_bor = zeros(par.JJ, par.NP);
    for j = 1:par.JJ
        for ip = 1:par.NP
            abor_temp = 0;
            for jj = par.JJ:-1:j
                abor_temp = abor_temp/(1+par.r) + ...
                    par.eff(jj)*par.theta(ip)*par.eta(1) + par.pen(jj);
            end
            abor_temp = min(-abor_temp/(1+par.r) + 1e-4, 0);
            a_bor(j, ip) = max(a_bor(j, ip), abor_temp);
        end
    end
end
...
function dist = get_distribution_mongey(sol, par, col)
    ...
    for ip = 1:par.NP
        % 初始 cohort 位于 eta 中间状态，初始资产为 -a_bor(1,ip)。
        % 用分布网格的线性 spline basis 把该点质量分配到相邻资产点。
        a0 = min(max(-par.a_bor(1, ip), col.a_dist(1)), col.a_dist(end));
        qx0 = funbas(col.fspace_d, a0);
        rows0 = (dist.is_initial-1)*col.nxd + (1:col.nxd);
        dist.L(rows0, ip, 1) = qx0(:)*par.dist_theta(ip);
    end

    dist.mass(1, :) = squeeze(sum(dist.L(:, :, 1), 1));
    dist.mass_error(1) = abs(sum(dist.mass(1, :)) - 1);
...
function agg = aggregation_mongey(sol, dist, par, col)
    ...
    % 这里的资产为 Fortran 口径 a + a_bor(j,ip)。
...
function output_results(sol, dist, agg, par, col)
    ...
    fid = fopen('output_collocation_policy.out', 'w');
    if fid < 0
        error('Could not open output_collocation_policy.out for writing.');
    end
    cleanup = onCleanup(@() fclose(fid));
    fprintf(fid, ' AGE    RESIDUAL       MIN_C      MAX_APLUS   UPPER_SHARE   EV_ERROR\n');
    for j = 1:par.JJ
        fprintf(fid, '%4d %12.4e %12.4e %12.4f %12.4e %12.4e\n', ...
            ages(j), residual(j), min_c(j), max_aplus(j), upper_share(j), ev_error(j));
    end
    clear cleanup

    fid = fopen('output_collocation_distribution.out', 'w');
    if fid < 0
        error('Could not open output_collocation_distribution.out for writing.');
    end
    cleanup = onCleanup(@() fclose(fid));
    fprintf(fid, ' AGE       MASS_ERR      CONS   EARNINGS     INCOME       PENS     ASSETS      CV_C      CV_Y  FRAC_BOR\n');
    for j = 1:par.JJ
        fprintf(fid, '%4d %12.4e %9.3f %10.3f %10.3f %10.3f %10.3f %9.3f %9.3f %9.3f\n', ...
            ages(j), dist.mass_error(j), agg.c_coh(j), agg.y_coh(j), agg.income_coh(j), ...
            agg.pen_coh(j), agg.a_coh(j), agg.cv_c(j), agg.cv_y(j), agg.frac_bor(j));
    end
    clear cleanup

    save('prog10_01_collocation_solution.mat', 'sol', 'dist', 'agg', 'par', 'col');
...
function make_policy_plots(sol, par, col, ages, residual)
    ...
    saveas(fig, 'collocation_policy_aplus.png');
    % close(fig);
    ...
    saveas(fig, 'collocation_policy_consumption.png');
    % close(fig);
    ...
    saveas(fig, 'collocation_value.png');
    % close(fig);
    ...
    saveas(fig, 'collocation_residual.png');
    % close(fig);
end
...
function make_distribution_plots(agg, dist, ages)
    ...
    title('Cohort means');
    saveas(fig, 'collocation_cohort_means.png');
    % close(fig);
    ...
    ylabel('Assets');
    title('Cohort assets');
    saveas(fig, 'collocation_cohort_assets.png');
    % close(fig);
    ...
    saveas(fig, 'collocation_cohort_cv.png');
    % close(fig);
    ...
    saveas(fig, 'collocation_borrowing_constrained.png');
    % close(fig);
    ...
    saveas(fig, 'collocation_distribution_mass_error.png');
    % close(fig);
end
