...
! function approximation (CompEcon style)
public :: func_space
public :: fundef, fundef_spli, fundef_cheb
public :: funnode
public :: funbas
public :: funeval
...
public :: spmat
public :: sp_eye, sp_diag, sparse, sp_kron
public :: sp_alloc_coo, sp_assemble
public :: sp_matmul, sp_transpose_matmul
public :: sp_to_dense, sp_free
public :: sp_build_mongey_Q
...
interface sort
    ...
    !
    ! Row i has nonzeros at columns colind(k) with values values(k) for
    ! k = rowptr(i), ..., rowptr(i+1)-1.  Column indices are sorted within
    ! each row.
    !##############################################################################
    type :: spmat
        integer :: m = 0       ! number of rows
        integer :: n = 0       ! number of columns
        integer :: nnz = 0     ! number of stored nonzeros
        integer, allocatable :: rowptr(:)   ! row pointers, length m+1
        integer, allocatable :: colind(:)   ! column indices, length nnz
        real*8,  allocatable :: values(:)   ! nonzero values, length nnz
    end type spmat
    ...
    ! TYPE func_space
    !
    ! Approximation space for function interpolation / collocation (CompEcon style).
    !
    !   basis = 'cheb'  → Chebyshev polynomials  T_0..T_n  on [left, right]
    !   basis = 'spli'  → B-splines of order spli_order on breakpoints breaks(0:)
    !
    !   n_basis  — number of basis functions
    !   nodes(:) — standard collocation nodes, filled by fundef / funnode
    !##############################################################################
    type :: func_space
        character(len=4) :: basis = 'spli'       ! 'spli' or 'cheb'
        integer :: n_basis = 0                    ! number of basis functions
        real*8  :: left   = 0d0                   ! interval lower bound (cheb)
        real*8  :: right  = 1d0                   ! interval upper bound (cheb)
        integer :: spli_order = 1                 ! 1 = linear spline, 3 = cubic spline
        real*8, allocatable :: breaks(:)          ! breakpoints, lower bound 0  (spli)
        real*8, allocatable :: nodes(:)           ! standard collocation nodes
    end type func_space
    ...
    ! INTERFACE fundef
    !
    ! Overloaded constructor for func_space:
    !   fundef(breaks, order)       → spline space
    !   fundef(n_cheb, left, right) → Chebyshev space
    !##############################################################################
    interface fundef
        module procedure fundef_spli, fundef_cheb
    end interface

    !##############################################################################
    ! INTERFACE funbas
    !
    ! Basis function evaluation, returns a CSR sparse matrix.  For spline
    ! spaces each row contains at most 2 (linear) or 4 (cubic) nonzeros;
    ! the matrix is built directly in CSR without intermediate dense storage.
    !##############################################################################
    interface funbas
        module procedure funbas_impl
    end interface
...
!
! CSR sparse matrix type and operations for efficient linear algebra on
! large, sparse matrices (e.g., collocation basis matrices, Kronecker
! products, Mongey transition matrices).
!##############################################################################
!##############################################################################
    ...
    ! FUNCTION sp_eye
    !
    ! Creates an n-by-n sparse identity matrix.
    !##############################################################################
    function sp_eye(n) result(S)
        ...
        if(n < 0)call error('sp_eye', 'matrix dimension must be nonnegative')

        S%m = n
        S%n = n
        S%nnz = n

        allocate(S%rowptr(n+1), S%colind(n), S%values(n))
    ...
    end function sp_diag


    !##############################################################################
    ! FUNCTION sparse
    !
    ! Converts a dense matrix to CSR sparse format.  Entries whose absolute
    ! value is below 1d-100 are treated as zero.
    !##############################################################################
    function sparse(A_dense) result(S)
        ...
        do i = 1, m
            do j = 1, n
                if(abs(A_dense(i, j)) > tol) S%nnz = S%nnz + 1
            enddo
        enddo

        allocate(S%rowptr(m+1), S%colind(S%nnz), S%values(S%nnz))

        ! second pass: fill CSR arrays (row-major order)
        k = 0
        S%rowptr(1) = 1
        do i = 1, m
            do j = 1, n
    ...
    !##############################################################################
    ! FUNCTION sp_transpose_matmul
    !
    ! Sparse matrix-transpose-vector multiplication  y = A' * x.
    !##############################################################################
    function sp_transpose_matmul(A, x) result(y)
        ...
        if(size(x, 1) /= A%m) &
            call error('sp_transpose_matmul', &
    ...
    end function sp_transpose_matmul


    !##############################################################################
    ! FUNCTION sp_to_dense
    !
    ! Converts a CSR sparse matrix to a dense 2-d array (useful for debugging).
    !##############################################################################
    function sp_to_dense(A) result(D)
        ...
        do i = 1, A%m
            do k = A%rowptr(i), A%rowptr(i+1)-1
                D(i, A%colind(k)) = A%values(k)
            enddo
        enddo

    end function sp_to_dense


    !##############################################################################
    ! SUBROUTINE sp_alloc_coo
    !
    ! Allocates COO (coordinate) triplet arrays for incremental sparse matrix
    ! construction.  After filling, call sp_assemble to convert to CSR.
    !##############################################################################
    subroutine sp_alloc_coo(est_nnz, rows, cols, vals)
        ...
        ! estimated number of nonzeros (capacity)
        integer, intent(in) :: est_nnz

        ! COO row indices (1-based)
        integer, allocatable, intent(out) :: rows(:)

        ! COO column indices (1-based)
        ...
        if(est_nnz <= 0) &
            call error('sp_alloc_coo', 'estimated nnz must be positive')

        allocate(rows(est_nnz), cols(est_nnz), vals(est_nnz))
        rows = 0
        cols = 0
        vals = 0d0

    end subroutine sp_alloc_coo


    !##############################################################################
    ! SUBROUTINE sp_assemble
    !
    ! Assembles a CSR sparse matrix from COO triplets.  The triplets are sorted
    ! by (row, col) and duplicate entries are merged by summation.
    ! On return the COO arrays are deallocated.
    !##############################################################################
    subroutine sp_assemble(m, n, rows, cols, vals, nentries, S)
        ...
        ! COO row indices (will be deallocated on return)
        integer, allocatable, intent(inout) :: rows(:)

        ! COO column indices (will be deallocated on return)
        integer, allocatable, intent(inout) :: cols(:)

        ! COO values (will be deallocated on return)
        real*8, allocatable, intent(inout) :: vals(:)

        ! actual number of triplets stored in the COO arrays
        integer, intent(in) :: nentries

        ! assembled CSR sparse matrix
        type(spmat), intent(out) :: S


        !##### OTHER VARIABLES ####################################################

        integer :: i, j, k, unique_nnz, rtmp, ctmp
        real*8 :: vtmp
        ...
        if(nentries <= 0) &
            call error('sp_assemble', 'nentries must be positive')
        if(m <= 0 .or. n <= 0) &
            call error('sp_assemble', 'matrix dimensions must be positive')

        ! ---- sort COO triplets by (row, col) using insertion sort ----
        do i = 2, nentries
            rtmp = rows(i)
            ctmp = cols(i)
            vtmp = vals(i)
            ...
            do while(j >= 1)
                if(rows(j) > rtmp .or. &
                   (rows(j) == rtmp .and. cols(j) > ctmp))then
                    rows(j+1) = rows(j)
                    cols(j+1) = cols(j)
                    vals(j+1) = vals(j)
                    j = j - 1
        ...
        unique_nnz = 0
        do i = 1, nentries
            if(i > 1 .and. rows(i) == rows(i-1) &
                         .and. cols(i) == cols(i-1))then
                vals(unique_nnz) = vals(unique_nnz) + vals(i)
            else
                unique_nnz = unique_nnz + 1
                rows(unique_nnz) = rows(i)
                cols(unique_nnz) = cols(i)
                vals(unique_nnz) = vals(i)
            endif
        enddo
        ...
        allocate(S%rowptr(m+1), S%colind(unique_nnz), S%values(unique_nnz))

        ! build rowptr
        S%rowptr(1) = 1
        k = 0
        do i = 1, m
            do while(k < unique_nnz .and. rows(k+1) <= i)
                k = k + 1
            enddo
            S%rowptr(i+1) = k + 1
        enddo

        ! copy colind and values
        do i = 1, unique_nnz
            S%colind(i) = cols(i)
            S%values(i) = vals(i)
        enddo
    ...
    end subroutine sp_assemble


    !##############################################################################
    ! FUNCTION sp_kron
    !
    ! Kronecker product of two CSR sparse matrices:  K = A \otimes B.
    !
    ! The output has dimensions  (A%m * B%m) \times (A%n * B%n) with
    ! A%nnz * B%nnz nonzeros, constructed in a single pass by exploiting
    ! the lexicographic structure of the Kronecker product.
    !##############################################################################
    function sp_kron(A, B) result(K)
        ...
        integer :: ia, ib, iak, ibk, out_row, kk
        real*8 :: val_a
        ...
        allocate(K%rowptr(K%m+1), K%colind(K%nnz), K%values(K%nnz))
        ...
        do ia = 1, A%m
            do ib = 1, B%m
                ...
                do iak = A%rowptr(ia), A%rowptr(ia+1)-1
                    val_a = A%values(iak)

                    do ibk = B%rowptr(ib), B%rowptr(ib+1)-1
                        kk = kk + 1
                        K%colind(kk) = (A%colind(iak)-1)*B%n + B%colind(ibk)
                        K%values(kk) = val_a * B%values(ibk)
                    enddo
                enddo

                K%rowptr(out_row+1) = kk + 1
            enddo
        enddo

    end function sp_kron


    !##############################################################################
    ! SUBROUTINE sp_build_mongey_Q
    !
    ! Builds the Mongey transition matrix Q in CSR format from the sparse
    ! asset-interpolation matrix QX (CSR) and the shock transition matrix pi.
    !
    ! QX is CSR-sparse  (nd \times nxd)  where each row contains the
    ! linear-spline interpolation weights (at most 2 nonzeros per row).
    ! pi is the dense  (NS \times NS)  shock transition matrix.
    ! The output Q is  (nd \times nd)  with nd = nxd * NS.
    !##############################################################################
    subroutine sp_build_mongey_Q(QX, pi, Q)
        ...
        ! asset interpolation matrix (nd x nxd), CSR sparse
        type(spmat), intent(in) :: QX

        ! shock transition matrix (NS x NS), dense
        real*8, intent(in) :: pi(:, :)

        ! Mongey transition matrix (CSR sparse)
        type(spmat), intent(out) :: Q


        !##### OTHER VARIABLES ####################################################

        integer :: nd, nxd, ns
        integer :: ia, is, isp, row, k, kk
        integer :: col_asset
        ...
        if(nd /= nxd*ns) &
            call error('sp_build_mongey_Q', &
                       'QX rows must equal nxd*NS')
        if(size(pi, 2) /= ns) &
            call error('sp_build_mongey_Q', &
                       'pi must be square (NS x NS)')

        ! each QX row has ≤ 2 nonzeros; each yields NS entries in Q
        Q%m   = nd
        Q%n   = nd
        Q%nnz = QX%nnz * ns
        allocate(Q%rowptr(nd+1), Q%colind(Q%nnz), Q%values(Q%nnz))
        ...
        do is = 1, ns
            do ia = 1, nxd
                ...
                do kk = QX%rowptr(row), QX%rowptr(row+1)-1
                    col_asset = QX%colind(kk)

                    do isp = 1, ns
                        k = k + 1
                        Q%colind(k) = (isp-1)*nxd + col_asset
                        Q%values(k) = pi(is, isp)*QX%values(kk)
                    enddo
                enddo
    ...
    end subroutine sp_build_mongey_Q


    !##############################################################################
    ! SUBROUTINE linbas_Gen
    !
    ! Computes the linear-spline basis matrix for arbitrary evaluation points on
    ! an irregular (possibly non-equidistant) grid.
    !
    ! For each evaluation point x(i), the routine locates the surrounding
    ! breakpoint interval  [xi(k), xi(k+1)]  and returns the two non-zero
    ! basis weights in Phi(i, k+1) and Phi(i, k+2).  Points outside the grid
    ! are clamped to the nearest boundary.
    !##############################################################################
    subroutine linbas_Gen(x, xi, Phi)
        ...
        ! breakpoint grid (lower bound 0)
        real*8, intent(in)  :: xi(0:)

        ! basis matrix:  Phi(i, k+1) = weight for breakpoint xi(k)
        real*8, intent(out) :: Phi(:, :)
        ...
        m = size(x, 1)
        n = size(xi, 1) - 1           ! number of intervals

        if(m /= size(Phi, 1) .or. n+1 /= size(Phi, 2)) &
            call error('linbas_Gen', 'Phi shape does not match (m, n+1)')
        ...
        do i = 1, m
            ...
            ! ---- interior: find interval  xi(k) <= x(i) <= xi(k+1) ----
            else
                do k = 0, n-1
                    if(x(i) >= xi(k) .and. x(i) <= xi(k+1))then
                        h = xi(k+1) - xi(k)
                        Phi(i, k+1) = (xi(k+1) - x(i)) / h
                        Phi(i, k+2) = (x(i) - xi(k))   / h
                        exit
                    endif
                enddo
            endif
    ...
    end subroutine linbas_Gen


    !##############################################################################
    ! FUNCTION lineval_Gen
    !
    ! Evaluates a linear-spline function from its basis coefficients on an
    ! irregular grid.  Equivalent to  y(i) = sum_j coef(j) * phi_j(x(i)).
    !##############################################################################
    function lineval_Gen(x, xi, coef) result(y)
        ...
        ! basis coefficients  coef(j) = function value at xi(j)
        real*8, intent(in) :: coef(0:)
    ...
    subroutine goldenx(f, low, high, xopt, fopt, itmax, tol)
        ...
        ! upper bounds (per element)
        real*8, intent(in)  :: high(:)

        ! maximisers
        real*8, intent(out) :: xopt(:)

        ! function values at the maximisers
        real*8, intent(out) :: fopt(:)

        ! (optional) maximal number of iterations
        integer, intent(in), optional :: itmax

        ! (optional) convergence tolerance on |b-a|
        ...
        if(nelem /= size(high, 1) .or. nelem /= size(xopt, 1) &
           .or. nelem /= size(fopt, 1)) &
            call error('goldenx', 'input/output arrays must have the same length')
...
! MODULE func_approx
!
! Function approximation in the style of Miranda & Fackler's CompEcon toolbox.
!
!   func_space  — encapsulates the basis (Chebyshev polynomial or B-spline),
!                 its interval / breakpoints, and standard collocation nodes.
!   fundef      — constructor (overloaded: spline or Chebyshev).
!   funnode     — returns the collocation nodes.
!   funbas      — evaluates the basis matrix at given points.
!   funeval     — evaluates a function from its basis coefficients.
!##############################################################################
!##############################################################################


    !##########################################################################
    ! FUNCTION fundef_spli
    !
    ! Constructs a B-spline function space on the given breakpoints.
    !
    !   breaks(0:)  — breakpoint array (lower bound 0)
    !   order       — 1 = linear spline,  3 = cubic spline
    !
    ! The number of basis functions is
    !   n_breaks + 1  (order = 1)
    !   n_breaks + 3  (order = 3, same convention as spline_interp1)
    !##########################################################################
    function fundef_spli(breaks, order) result(fspace)
        ...
        ! breakpoint array  xi(0:n)
        real*8, intent(in) :: breaks(0:)

        ! spline order (1 = linear, 3 = cubic)
        integer, intent(in) :: order

        ! the function space
        type(func_space) :: fspace
        ...
        if(order /= 1 .and. order /= 3) &
            call error('fundef_spli', 'order must be 1 (linear) or 3 (cubic)')

        n_breaks = size(breaks, 1) - 1

        fspace%basis      = 'spli'
        fspace%spli_order  = order
        fspace%left       = breaks(0)
        fspace%right      = breaks(n_breaks)

        if(order == 1)then
            fspace%n_basis = n_breaks + 1
        else
            fspace%n_basis = n_breaks + 3
        endif

        allocate(fspace%breaks(0:n_breaks))
        fspace%breaks = breaks

        ! collocation nodes = breakpoints
        allocate(fspace%nodes(fspace%n_basis))
        fspace%nodes(1:n_breaks+1) = breaks

        ! for cubic: append two extra nodes (extend linearly at boundaries)
        if(order == 3)then
            fspace%nodes(n_breaks+2) = breaks(n_breaks) &
                + (breaks(n_breaks) - breaks(n_breaks-1))
            fspace%nodes(n_breaks+3) = breaks(n_breaks) &
                + 2d0*(breaks(n_breaks) - breaks(n_breaks-1))
        endif

    end function fundef_spli


    !##########################################################################
    ! FUNCTION fundef_cheb
    !
    ! Constructs a Chebyshev polynomial function space of degree n on [a,b].
    !
    !   n          — degree (→ n+1 basis functions  T_0,...,T_n)
    !   left,right — interval
    !
    ! Collocation nodes are the Chebyshev extrema  cos(k*pi/n), k=0..n,
    ! mapped to [left, right].
    !##########################################################################
    function fundef_cheb(n, left, right) result(fspace)
        ...
        ! Chebyshev degree (→ n+1 basis functions)
        integer, intent(in) :: n
        ...
        if(n < 1)call error('fundef_cheb', 'degree must be >= 1')
        if(left >= right)call error('fundef_cheb', &
        ...
        allocate(fspace%nodes(n+1))
        do k = 0, n
            fspace%nodes(k+1) = (left+right)/2d0 + (right-left)/2d0 &
                * cos(dble(n-k)*pi/dble(n))
        enddo

    end function fundef_cheb


    !##########################################################################
    ! FUNCTION funnode
    !
    ! Returns the standard collocation nodes of a function space.
    !##########################################################################
    function funnode(fspace) result(nodes)
        ...
        ! function space
        type(func_space), intent(in) :: fspace

        ! collocation nodes
        real*8 :: nodes(fspace%n_basis)
    ...
    end function funnode


    !##########################################################################
    ! FUNCTION funbas_impl
    !
    ! Basis matrix in CSR sparse format.  For spline spaces the matrix is
    ! constructed directly in CSR without intermediate dense storage.
    ! For Chebyshev polynomials all n_basis entries are stored per row.
    !##########################################################################
    function funbas_impl(fspace, x) result(S)
        ...
        ! sparse basis matrix (CSR)
        type(spmat) :: S


        !##### ROUTINE CODE ###################################################

        if(fspace%basis == 'spli')then
            call bas_spli_csr(fspace, x, S)
        elseif(fspace%basis == 'cheb')then
            call bas_cheb_csr(fspace, x, S)
        else
            call error('funbas', 'unknown basis type: ' // fspace%basis)
        endif

    end function funbas_impl


    !##########################################################################
    ! SUBROUTINE bas_spli_csr
    !
    ! Constructs the B-spline basis matrix directly in CSR format.
    !
    ! Each row has at most  2*order  nonzeros (2 for linear, 4 for cubic).
    ! The CSR arrays are filled in a single pass over the evaluation points.
    !##########################################################################
    subroutine bas_spli_csr(fspace, x, S)
        ...
        integer :: m, i, order, n_int, n_coef, k, jj, j, p, nnz_est
        real*8  :: xi_low, xi_up, t, d, h, val


        !##### ROUTINE CODE ###################################################

        m     = size(x, 1)
        order = fspace%spli_order
        n_int = size(fspace%breaks, 1) - 1
        n_coef = fspace%n_basis

        ! pre-allocate: at most 2*order entries per row
        nnz_est = m * (2*order)
        S%m   = m
        S%n   = n_coef
        S%nnz = nnz_est
        allocate(S%rowptr(m+1), S%colind(nnz_est), S%values(nnz_est))
        ...
        do i = 1, m
            ...
            ! ---- map x(i) to index coordinate t ----
            if(x(i) <= fspace%breaks(0))then
                t = 0d0
            elseif(x(i) >= fspace%breaks(n_int))then
                t = dble(n_int)
            else
                do k = 0, n_int-1
                    xi_low = fspace%breaks(k)
                    xi_up  = fspace%breaks(k+1)
                    if(x(i) >= xi_low .and. x(i) <= xi_up)then
                        h = xi_up - xi_low
                        t = dble(k) + (x(i) - xi_low)/h
                        exit
                    endif
                enddo
            endif
            ...
            if(order == 1)then
                ...
                k = floor(t)
                if(k < 0) k = 0
                if(k > n_int-1) k = n_int-1

                val = dble(k+1) - t
                if(abs(val) > 1d-100)then
                    p = p + 1; S%colind(p) = k+1; S%values(p) = val
                endif
                val = t - dble(k)
                if(abs(val) > 1d-100)then
                    p = p + 1; S%colind(p) = k+2; S%values(p) = val
                endif
            else
                ! cubic: up to 4 nonzeros, columns floor(t)+1 .. floor(t)+4
                k = floor(t)
                do jj = k+1, k+4
                    j = min(max(jj, 1), n_coef)
                    d = abs(t - dble(j-2))
                    if(d <= 1d0)then
                        val = 4d0 + d*d*(3d0*d - 6d0)
                    elseif(d <= 2d0)then
                        val = (2d0 - d)**3
                    else
                        cycle
                    ...
                    if(abs(val) > 1d-100)then
                        p = p + 1; S%colind(p) = j; S%values(p) = val
                    endif
                enddo
            endif
    ...
    end subroutine bas_spli_csr


    !##########################################################################
    ! SUBROUTINE bas_cheb_csr
    !
    ! Chebyshev polynomial basis matrix in CSR.  Since Chebyshev polynomials
    ! are globally supported, every row has n_basis entries (= "dense" CSR;
    ! this is still available via the unified sparse API).
    !##########################################################################
    subroutine bas_cheb_csr(fspace, x, S)
        ...
        do i = 1, m
            z = 2d0*(x(i) - fspace%left)/(fspace%right - fspace%left) - 1d0
            z = min(max(z, -1d0), 1d0)
            z2 = 2d0*z

            ! T_0
            p = p + 1; S%colind(p) = 1; S%values(p) = 1d0

            if(n >= 2)then
                p = p + 1; S%colind(p) = 2; S%values(p) = z

                Tk2 = 1d0; Tk1 = z
                do k = 2, n-1
                    Tk = z2*Tk1 - Tk2
                    p = p + 1; S%colind(p) = k+1; S%values(p) = Tk
                    Tk2 = Tk1; Tk1 = Tk
                enddo
            endif
            S%rowptr(i+1) = p + 1
    ...
    end subroutine bas_cheb_csr


    !##########################################################################
    ! FUNCTION funeval
    !
    ! Evaluates a function from its basis coefficients.
    !
    !   y(i) = sum_{j=1}^{n_basis}  coeff(j) * phi_j(x(i))
    !
    ! For Chebyshev: uses Clenshaw's recurrence (stable).
    ! For splines:  linear → lineval_Gen
    !               cubic   → spline1_grid (reuses toolbox)
    !##########################################################################
    function funeval(fspace, coeff, x) result(y)
        ...
        ! basis coefficients  (length = n_basis)
        ...
        integer  :: i, k, n
        real*8   :: z, z2, d_k, d_k1, d_k2


        !##### ROUTINE CODE ###################################################

        if(size(coeff, 1) /= fspace%n_basis) &
            call error('funeval', 'coeff must have length n_basis')

        if(fspace%basis == 'spli' .and. fspace%spli_order == 1)then
            ! ---- linear spline: delegate to lineval_Gen ----
            y = lineval_Gen(x, fspace%breaks, coeff)

        elseif(fspace%basis == 'spli' .and. fspace%spli_order == 3)then
            ! ---- cubic spline: reuse spline1_grid ----
            if(.not. allocated(fspace%breaks)) &
                call error('funeval', 'breaks not allocated')
            do i = 1, size(x)
                y(i) = spline1_grid(x(i), coeff, &
                       fspace%left, fspace%right)
            enddo

        elseif(fspace%basis == 'cheb')then
            ! ---- Chebyshev: Clenshaw recurrence ----
            n = fspace%n_basis - 1
            do i = 1, size(x)
                z = 2d0*(x(i) - fspace%left) &
                    /(fspace%right - fspace%left) - 1d0
                z = min(max(z, -1d0), 1d0)
                z2 = 2d0*z
                ...
                d_k1 = 0d0
                do k = n, 1, -1
                    d_k  = coeff(k+1) + z2*d_k1 - d_k2
                    d_k2 = d_k1
                    d_k1 = d_k
                enddo
                y(i) = coeff(1) + z*d_k1 - d_k2
            enddo

        else
            call error('funeval', 'unknown basis or unsupported order')
        endif
    ...
    ! SUBROUTINE sp_free
    !
    ! Deallocates the internal storage of a sparse matrix.
    !##############################################################################
    subroutine sp_free(A)
